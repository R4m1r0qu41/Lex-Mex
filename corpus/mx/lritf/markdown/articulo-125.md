---
id: urn:lex-mx:federal:statute:lritf:article:125
instrument_id: urn:lex-mx:federal:statute:lritf
instrument: LRITF
name: "Ley para Regular las Instituciones de Tecnología Financiera"
provision_type: article
number: "125"
aliases: ["LRITF — Artículo 125"]
generated: true
temporal_status: effective
review_status: not_analyzed
source_url: https://www.diputados.gob.mx/LeyesBiblio/pdf/LRITF.pdf
source_sha256: d6f645e6a7d3c2eeb46905d4d24ecd8e078907057dc034cda715abf019ce8491
---

> Título VI · Capítulo II

# Artículo 125

Será sancionado con pena de prisión de siete a quince años y multa de 5,000 a 150,000 [UMA](articulo-4.md) quien para obtener la autorización para operar como [ITF](articulo-4.md) o con [Modelos Novedosos](articulo-4.md) o con activos virtuales, proporcione información falsa a la autoridad financiera que corresponda.
